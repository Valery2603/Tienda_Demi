...
if(isset($_POST['nombre'])){
    ...
    if(empty($errores)){   
        $consultaupdate="UPDATE categorias SET nombre_categoria='$nombre',descripcion='$descripcion' WHERE id_categoria=$id";    
        $resultadoup=mysqli_query($db,$consultaupdate);
        if($resultadoup){
            header('Location: listacategoria.php?resultado=2');
        }           
    } 
    if (!empty($errores)) {
        session_start();
        $_SESSION['errores_registro'] = $errores;
        header("Location: actualizarcategoria.php?id=$id"); 
        echo $errores;
        exit();
    }   
}
...
<body class='fondo adamina-regular'>
    ...
    <main class="seccion">
        <h1 class='mt-5 mb-3 text-center'>Actualizar Categoría</h1>
        <?php
